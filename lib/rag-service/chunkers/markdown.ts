/**
 * Strategia di chunking markdown (heading h2/h3, liste con preambolo).
 * Logica spostata da chunker.ts senza modifiche comportamentali.
 */
...
export const MARKDOWN_STRATEGY_VERSION = "markdown-v2.1";

export const MIN_LIST_ITEMS_FOR_SPLIT = 3;
export const MIN_BULLET_CHARS = 50;
export const MAX_PREAMBLE_CHARS = 300;
export const MAX_LIST_PARENT_HEADING_CHARS = 200;
...
function buildChunkMetadata(meta: ChunkerMetadata, chunkerVersion: string): ChunkMetadata {
  return {
    ...meta,
    chunkerVersion,
...
export class MarkdownChunkStrategy implements ChunkStrategy {
  readonly name = "markdown";
  readonly version = MARKDOWN_STRATEGY_VERSION;

  matches(filename: string): boolean {
    return filename.endsWith(".md") || filename.endsWith(".markdown");
  }

  chunk(markdown: string, ctx: ChunkStrategyContext): Chunk[] {
    ...
    const pushChunks = (section: string, body: string) => {
      ...
      for (const part of parts) {
        ...
        chunks.push({
          ...
          section,
          content: part,
          metadata: buildChunkMetadata(metadata, this.version),
        });
      }
    };
